    ...
    namespace External
    {
        ...
        */
        namespace Debug
        {
            ...
            {
            public:
                static ECAPI std::function<void(const std::string&, const std::string&, unsigned int)> Debug_Pointer;
                static ECAPI void Debug(const std::string& message, const std::string& function = "", unsigned int line = -1)
                {
                    if (Debug_Pointer) { Debug_Pointer(message, function, line); }
                    else std::cout << message << " " << function << " " << line << std::endl;
                }
                static ECAPI std::function<void(const std::string&, const std::string&, unsigned int)> Error_Pointer;
                static ECAPI void Error(const std::string& message, const std::string& function = "", unsigned int line = -1)
                {
                    if (Error_Pointer) { Error_Pointer(message, function, line); }
                    else std::cout << message << " " << function << " " << line << std::endl;
                }
                static ECAPI std::function<void(const std::string&, const std::string&, unsigned int)> Log_Pointer;
                static ECAPI void Log(const std::string& message, const std::string& function = "", unsigned int line = -1)
                {
                    if (Log_Pointer) { Log_Pointer(message, function, line); }
                    else std::cout << message << " " << function << " " << line << std::endl;
                }
            };
        }
